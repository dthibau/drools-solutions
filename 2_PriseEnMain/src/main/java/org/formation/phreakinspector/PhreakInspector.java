 ...
 */
package org.formation.phreakinspector;
...
//        }
    ...
    private void visitSink(Sink sink, Node parentNode) {
        if (sink instanceof LeftInputAdapterNode) {
        	...
        	TimerNode timer = (TimerNode)sink;
        	this.visitTimerNode(timer, parentNode);
        } else if (sink instanceof ExistsNode) {
        	ExistsNode en = (ExistsNode)sink;      	
        	this.visitExistNode(en, parentNode);
        } else {
            throw new UnsupportedOperationException(sink.toString());
        }
    }
    private void visitExistNode(ExistsNode existNode, Node parentNode) {
    	LeftTupleSink[] ltSinks = existNode.getSinkPropagator().getSinks();
    	for (LeftTupleSink ltSink : ltSinks) {
            visitLeftTupleSink(ltSink, parentNode);
        }
    }
    private void visitTimerNode(TimerNode timerNode, Node parentNode) {
    	LeftTupleSink[] ltSinks = timerNode.getSinkPropagator().getSinks();
    	for (LeftTupleSink ltSink : ltSinks) {
            visitLeftTupleSink(ltSink, parentNode);
        }
    }
    ...
    private void visitRightInputAdapterNode(RightInputAdapterNode rian, Node parentNode) {
        LeftTupleSink[] oSinks = rian.getSinkPropagator().getSinks();
        for (LeftTupleSink oSink : oSinks) {
            this.visitLeftTupleSink(oSink, parentNode);
        }
    }
